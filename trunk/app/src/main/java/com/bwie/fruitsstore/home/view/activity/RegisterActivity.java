...
public class RegisterActivity extends BaseActivity {
    ...
    @BindView(R.id.relat_register)
    RelativeLayout mRelatRegister;
    ...
    @OnClick({R.id.register_back, R.id.getyzm, R.id.next})
...
//                                startActivity(new Intent(RegisterActivity.this, LoginActivity.class)); //页面跳转
                ...
                if (!mEtPhone.getText().toString().trim().equals("")) {
                    if (checkTel(mEtPhone.getText().toString().trim())) {
                        ...
                        makeText(RegisterActivity.this, "请输入正确的手机号码", Toast.LENGTH_SHORT).show();
                    }
                } else {
                    makeText(RegisterActivity.this, "请输入手机号码", Toast.LENGTH_SHORT).show();
                }
                break;
            case R.id.next:
                if (!mEtPhone.getText().toString().trim().equals("")) {
                    if (checkTel(mEtPhone.getText().toString().trim())) {
                        if (!mEtPhoneCode.getText().toString().trim().equals("")) {
                            ...
                            if (!mEtPassword.getText().toString().trim().equals("")) {
                                if (!mEtInviteCode.getText().toString().trim().equals("")) {
                                    ...
                                    Toast toast = makeText(RegisterActivity.this, "请输入客户邀请码", Toast.LENGTH_SHORT);
                                    Utils_Toast.showMyToast(toast, 1000);
                                }
                            } else {
                                Toast toast = makeText(RegisterActivity.this, "密码格式有误", Toast.LENGTH_SHORT);
                                Utils_Toast.showMyToast(toast, 1000);
                            }
                        } else {
                        ...
                        Toast toast = makeText(RegisterActivity.this, "请输入正确的手机号码", Toast.LENGTH_SHORT);
                        Utils_Toast.showMyToast(toast, 1000);
                    }
                } else {
                    Toast toast = makeText(RegisterActivity.this, "请输入手机号码", Toast.LENGTH_SHORT);
                    Utils_Toast.showMyToast(toast, 1000);
                }
                break;
        }
